...
# -*- coding: utf-8 -*-
"""
MNI — Macro News Intelligence Agent v1.2
...
APP_VERSION = "MNI-MACRO-NEWS-INTELLIGENCE-V1.2"
...
ENV_RSS_URLS = [x.strip() for x in os.getenv("MNI_RSS_URLS", "").split(",") if x.strip()]
RSS_URLS = list(dict.fromkeys(DEFAULT_RSS_URLS + ENV_RSS_URLS))
...
STATE = MNIState()
app = FastAPI(title="MNI Macro News Intelligence", version="1.2")
...
def source_priority(url: str) -> int:
    u = (url or "").lower()
    if "bls.gov" in u or "federalreserve.gov" in u:
        return 100
    return 60

def source_status() -> Dict[str, Any]:
    return {
        "official_default_sources": DEFAULT_RSS_URLS,
        "extra_sources": ENV_RSS_URLS,
        "rss_sources_total": len(RSS_URLS),
...
async def startup() -> None:
    ...
    print(f"[MNI SOURCES] official={len(DEFAULT_RSS_URLS)} extra={len(ENV_RSS_URLS)} total={len(RSS_URLS)}", flush=True)
